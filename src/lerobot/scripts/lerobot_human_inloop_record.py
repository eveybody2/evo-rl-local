...
def human_inloop_record(cfg: RecordConfig):
    ...
    # 只要有本地 policy 或者指定了远程 server_address，就激活人在环接管状态机
    has_policy = (cfg.policy is not None) or (getattr(cfg, 'server_address', None) is not None)

    cfg.policy_sync_to_teleop = has_policy
    cfg.intervention_state_machine_enabled = has_policy
    ...
    # 增加：开启 subtask 字段录制
    cfg.enable_subtask_labeling = True
    ...
    print("=" * 50)
    print(f"[DEBUG 1] 准备进入 record()。")
    print(f"[DEBUG 1] 当前 cfg.policy_sync_to_teleop = {cfg.policy_sync_to_teleop}")
    print(f"[DEBUG 1] 当前 server_address = {getattr(cfg, 'server_address', None)}")
    print("=" * 50)
    return record(cfg)
