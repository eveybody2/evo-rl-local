...
class RecordConfig:
    ...
    # 增加：是否通过键盘按键 (1, 2, 3) 捕获子任务标签
    enable_subtask_labeling: bool = False
...
def record(cfg: RecordConfig) -> LeRobotDataset:
    ...
    # 增加：注册 subtask 特征
    if cfg.enable_subtask_labeling:
        dataset_features["subtask"] = {
            "dtype": "int64",
            "shape": (1,),
            "names": None,
        }
    ...
    remote_bridge = None  # 初始化 bridge
    print(f"[DEBUG 2.0] 进入 record() 内部，当前 cfg.policy_sync_to_teleop = {cfg.policy_sync_to_teleop}")
    try:
        if cfg.resume:
        ...
        # 如果传入了远程服务器地址，则跳过本地模型加载
        if cfg.server_address:
            from lerobot.async_inference.helpers import map_robot_keys_to_lerobot_features
            from lerobot.scripts.recording_loop import RemotePolicyBridge

            robot_features = map_robot_keys_to_lerobot_features(robot)

            remote_bridge = RemotePolicyBridge(
                server_address=cfg.server_address,
                policy_type=cfg.policy_type,
                pretrained_name_or_path=cfg.pretrained_name_or_path,
                lerobot_features=robot_features,
                policy_device=cfg.policy_device,
                rename_map=cfg.dataset.rename_map  # <--- 🔴 加上这行，将参数透传下去
            )
            # 用一个字符串占位符欺骗人在环状态机，让它认为模型存在，从而允许按下 'i' 键接管
            policy = "REMOTE_POLICY_ACTIVE"
            logging.info(f"Connected to remote policy server at {cfg.server_address}")

        elif cfg.policy is not None:
            # 走原本的本地加载逻辑
            policy = make_policy(cfg.policy, ds_meta=dataset.meta)
        # ... 保持 make_pre_post_processors 原始代码不动 ...
        ...
        if cfg.policy_sync_to_teleop or getattr(cfg, 'server_address', None):
            # 只要本地 policy 存在，或者远程 server_address 存在，就放行
            print("[DEBUG 2.1] 满足条件，准备创建 PolicySyncDualArmExecutor！")
            if cfg.policy is None and not getattr(cfg, 'server_address', None):
                raise ValueError("`policy_sync_to_teleop=true` requires `policy` (or server_address) to be set.")
            ...
            print(f"[DEBUG 2.2] 创建成功！policy_sync_executor = {policy_sync_executor}")
        else:
            print("[DEBUG 2.3] ⚠️ 警告：跳过了创建 policy_sync_executor！")
        ...
        with VideoEncodingManager(dataset):
            ...
            while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
                ...
                # 增加：每个新 episode 开始时，强制重置 subtask 为 1
                events["subtask"] = 1
                ...
                record_loop(
                    ...
                    remote_bridge=remote_bridge,  # <---- 把 bridge 传进去
                    dataset=dataset,
