...
class VLAJEPAPolicy(PreTrainedPolicy):
    ...
    def get_optim_params(self) -> list[dict]:
        """
        覆盖默认方法，实现“大脑、小脑、世界模型”的分层学习率 (Layer-wise Learning Rate)。
        采用严格名称匹配，防止冻结层意外泄露参与训练。
        """
        vlm_params = []
        action_params = []
        predictor_params = []
        ...
        for name, param in self.model.named_parameters():
            ...
            elif "video_predictor" in name:  # 精准锁定世界模型，避开 video_encoder
                predictor_params.append(param)
            else:
                # 极端情况下的安全兜底：
                # 如果有不在以上三大模块中的参数意外处于 requires_grad=True 状态
                # 会被隔离到这里。由于我们用了精准的 PEFT 正则和 full_training_modules，
                # 正常情况下这个 other_params 列表应该是空的。
                other_params.append(param)

        # 获取配置中的分层学习率
        lr_base = self.config.optimizer_lr
        lr_vlm = getattr(self.config, "optimizer_lr_vlm", lr_base)
        lr_action = getattr(self.config, "optimizer_lr_action", lr_base)

        if lr_vlm is None: lr_vlm = lr_base
        if lr_action is None: lr_action = lr_base

        # 构建 PyTorch 优化器参数组
        optim_groups = []
        if vlm_params:
            optim_groups.append({"params": vlm_params, "lr": lr_vlm})
        if action_params:
            optim_groups.append({"params": action_params, "lr": lr_action})
        if predictor_params:
            optim_groups.append({"params": predictor_params, "lr": lr_base})
        if other_params:
            # 即使有漏网之鱼，也用最小的 base_lr 给它，防止梯度爆炸
            optim_groups.append({"params": other_params, "lr": lr_base})
