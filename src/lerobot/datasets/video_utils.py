...
def decode_video_frames_torchvision(
    ...
    if not is_within_tol.all():
        # 🛡️ 【Torchvision 时间戳防崩补丁】
        logging.warning(
            f"[Torchvision Timestamp Fix] Query timestamp exceeded tolerance ({min_.max().item():.4f} > {tolerance_s}). "
            f"Likely near video end. Clamping to closest available frame to avoid crash."
        )
...
def decode_video_frames_torchcodec(
    ...
    # 🛡️ 【核心卡死补丁】：通过 min() 强行把计算出的每一帧索引卡在 [0, 视频物理总帧数 - 1] 的安全范围内
    frame_indices = [min(round(ts * average_fps), metadata.num_frames - 1) for ts in timestamps]
    ...
    is_within_tol = min_ < tolerance_s
    # if not is_within_tol.all():
    #     # 🛡️ 【时间戳对齐防崩补丁】：如果超出了极其严苛的 0.0001s 容忍度，打印警告但绝不中断中断
    #     logging.warning(
    #         f"[Timestamp Fix] Query timestamp exceeded tolerance ({min_.max().item():.4f} > {tolerance_s}). "
    #         f"Likely near video end. Clamping to closest available frame to avoid crash."
    #     )
