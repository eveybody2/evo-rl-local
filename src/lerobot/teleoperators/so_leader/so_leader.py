...
class SOLeader(Teleoperator):
    ...
    def send_feedback(self, feedback: dict[str, float]) -> None:
        ...
        # 1. 提取传入的动作目标位（去掉 .pos 后缀）
        raw_goal_pos = {key.removesuffix(".pos"): val for key, val in feedback.items() if key.endswith(".pos")}



        leader_motor_names = list(self.bus.motors.keys())

        goal_pos = {}
        for clean_key, val in raw_goal_pos.items():
            match_found = False
            for motor_name in leader_motor_names:
                # 只要远端传来的名字（比如 follower_shoulder）以主臂的电机名（shoulder）结尾，就算匹配成功
                if clean_key.endswith(motor_name):
