...
def count_lora_params(model):
    ...
    for name, param in model.named_parameters():
        ...
        # PEFT规则：带 lora_A / lora_B 是LoRA权重，且requires_grad=True
        if ("lora_A" in name or "lora_B" in name) and param.requires_grad:
            lora_trainable_params += param_num

    # 计算占比
    ratio = lora_trainable_params / total_params * 100

    print("=" * 60)
    print(f"模型全部原始总参数量: {total_params:,}")
    print(f"LoRA可训练参数量:    {lora_trainable_params:,}")
    print(f"LoRA参数占全部模型:  {ratio:.4f} %")
    print("=" * 60)
